			      ...
			      size_t count, loff_t *offset)
{
	...
	if (!count) {
		dev_dbg(afe->dev, "%s(), count is 0, return directly\n",
			 __func__);
		goto exit;
	}
	...
	str_begin = kstrndup(input, MAX_DEBUG_WRITE_INPUT - 1,
			     GFP_KERNEL);
	if (!str_begin) {
		dev_dbg(afe->dev, "%s(), kstrdup fail\n", __func__);
		goto exit;
	}
	temp = str_begin;
	...
	dev_dbg(afe->dev, "%s(), command %s, content %s\n",
