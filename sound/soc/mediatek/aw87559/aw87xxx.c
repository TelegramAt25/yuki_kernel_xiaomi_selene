...
static char aw87xxx_cfg_name[AW87XXX_MODE_MAX][AW87XXX_CFG_NAME_MAX] = {
	{"aw87559_pid_xx_off_"},
	{"aw87559_pid_xx_music_"},
	{"aw87559_pid_xx_voice_"},
	{"aw87559_pid_xx_fm_"},
	{"aw87559_pid_xx_rcv_"},
};
...
/*K19A code for WXYFB-1013 by xuqingli at 2021.4.25 start*/
	...
	aw_dev_dbg(aw87xxx->dev, "%s :g_reg_update_cnt=%d\n", __func__, g_reg_update_cnt);
/*K19A code for WXYFB-1013 by xuqingli at 2021.4.25 end*/
}
				  ...
				  struct aw87xxx_scene_param *scene_param)
{
	...
	aw_dev_dbg(aw87xxx->dev, "%s enter, mode:%d\n",
				    ...
				    uint32_t *chip_name, uint32_t len)
{
	...
	aw_dev_dbg(aw87xxx->dev, "%s: chip name:%s\n",
				    ...
				    const struct firmware *cont)
{
	...
	aw_dev_dbg(aw87xxx->dev, "%s: check_sum:%d sum_data:%d\n",
		    __func__, check_sum, sum_data);
	if (check_sum != sum_data) {
		aw_dev_dbg(aw87xxx->dev, "%s: use old parsing\n", __func__);
		return AW87XXX_OLD_FIRMWARE;
	} else {
		aw_dev_dbg(aw87xxx->dev, "%s: use frame head parsing\n",
			    __func__);
					   ...
					   const struct firmware *cont)
{
	...
	aw_dev_dbg(aw87xxx->dev, "%s: reg_byte_len:0x%08x\n", __func__,
		    current_scene->bin_file->header_info[0].reg_byte_len);
	aw_dev_dbg(aw87xxx->dev, "%s: data_byte_len:0x%08x\n", __func__,
		    current_scene->bin_file->header_info[0].data_byte_len);
	...
	aw_dev_dbg(aw87xxx->dev, "%s: %s update complete\n",
				   ...
				   const struct firmware *cont)
{
	...
	if (ret == AW87XXX_OLD_FIRMWARE) {
		...
		aw_dev_dbg(aw87xxx->dev, "%s: old bin file", __func__);
	} else if (ret == AW87XXX_FH_FIRMWARE) {
		aw87xxx_parse_add_head_firmware(aw87xxx, current_scene, cont);
		aw_dev_dbg(aw87xxx->dev, "%s: new bin file", __func__);
	}
...
static void aw87xxx_scene_cfg_loaded(const struct firmware *cont, void *context)
{
	...
	aw_dev_dbg(aw87xxx->dev, "%s: loaded %s - size: %zu\n",
		    __func__, aw87xxx->cfg_name[current_scene->scene_mode],
		    cont ? cont->size : 0);
...
static void aw87xxx_vmax_cfg_loaded(const struct firmware *cont, void *context)
{
	...
	if (!vmax_cfg) {
		aw_dev_dbg(aw87xxx->dev, "%s: vmax_cfg kzalloc failed\n",
			   __func__);
		return;
	}
	...
	for (i = 0; i < cont->size; i += 8) {
		...
		aw_dev_dbg(aw87xxx->dev, "%s: minthr:%d ,vmax:0x%08x\n",
	...
	aw_dev_dbg(aw87xxx->dev, "%s: vbat monitor update complete\n",
		    __func__);
}
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	aw_dev_dbg(aw87xxx->dev, "%s enter, i2c%d@0x%02X, channel = %d\n",
					...
					struct device_node *np)
{
	...
	if (aw87xxx->reset_gpio < 0) {
		...
		aw_dev_dbg(aw87xxx->dev, "%s: reset gpio provided ok\n",
			 __func__);
	}
}
...
static void aw87xxx_int_to_str(struct aw87xxx *aw87xxx)
{
	...
	if (temp_channel >= 0) {
		snprintf(aw87xxx->channel, sizeof(aw87xxx->channel) + 1, "%d",
			 temp_channel);
		aw_dev_dbg(aw87xxx->dev, "%s: channel: %s", __func__,
					aw87xxx->channel);
	} else {
		aw87xxx->channel[0] = '-';
				     ...
				     struct device_node *np)
{
	...
	if (ret != 0) {
		...
		aw_dev_dbg(aw87xxx->dev, "%s: pa-channel: %d\n",
	...
	for (i = 0; i < sizeof(aw87xxx->cfg_name) / AW87XXX_CFG_NAME_MAX; i++) {
		if (aw87xxx->channel[0] != '-') {
			...
			memcpy(aw87xxx->cfg_name[i] + cfg_size, aw87xxx_end,
			       strlen(aw87xxx_end));
			aw_dev_dbg(aw87xxx->dev, "%s:cfg_name: %s\n",
...
static void aw87xxx_softreset(struct aw87xxx *aw87xxx)
{
	...
	aw_dev_dbg(aw87xxx->dev, "%s: softreset, 0x%x = 0x%x", __func__,
