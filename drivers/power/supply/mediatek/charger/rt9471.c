...
static void rt9471_inform_psy_dwork_handler(struct work_struct *work)
{
	...
	dev_dbg(chip->dev, "%s attach = %d, type = %d\n", __func__,
			    attach, chg_type);
...
static void rt9471_bc12_done_handler(struct rt9471_chip *chip)
{
	...
	chg_rdy = (regval & RT9471_ST_CHGRDY_MASK ? true : false);
	dev_dbg(chip->dev, "%s bc12_done = %d, chg_rdy = %d, chip_rev = %d\n",
...
static int rt9471_parse_dt(struct rt9471_chip *chip)
{
	...
	if (!np) {
		dev_dbg(chip->dev, "%s no rt9471 device node\n", __func__);
		np = parent_np;
	}
	...
	if (ret < 0)
		dev_dbg(chip->dev, "%s no rm-dev-addr(%d)\n", __func__, ret);
					  ...
					  bool is_inc)
{
	...
	dev_dbg(chip->dev, "%s is_inc = %d, chip_rev = %d\n",
					    ...
					    u32 uV)
{
	...
	dev_dbg(chip->dev, "%s target = %d, chip_rev = %d\n",
