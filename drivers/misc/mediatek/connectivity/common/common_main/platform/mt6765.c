...
static INT32 consys_hw_vcn18_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		...
		if (!(wmt_lib_get_ext_ldo())) {
			...
			WMT_PLAT_PR_DBG("VCN33 uses external LDO!\n");
	} else {
		if (!(wmt_lib_get_ext_ldo())) {
			if (reg_VCN33_BT)
...
#endif
			WMT_PLAT_PR_DBG("WMT do BT/WIFI v3.3 on\n");
			gBtWifiV33.counter++;
		}
...
#endif
			WMT_PLAT_PR_DBG("WMT do BT/WIFI v3.3 off\n");
			gBtWifiV33.counter--;
		} else if (gBtWifiV33.counter == 2) {
			gBtWifiV33.counter--;
...
static UINT32 consys_emi_set_remapping_reg(VOID)
{
	...
	WMT_PLAT_PR_DBG("before CONSYS_EMI_MAPPING read:0x%zx, value:0x%x\n",
...
static INT32 consys_dump_osc_state(P_CONSYS_STATE state)
{
	...
	if (addr) {
		...
		WMT_PLAT_PR_DBG("[%s] [%x][%x] [%x][%x] [%x] [%x][%x]", __func__,
