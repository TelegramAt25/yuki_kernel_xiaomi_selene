...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV8856_OFILM_FRONT_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
{
	LOG_INF("swl streaming_enable(0=Sw Standby,1=streaming): %d\n", enable);
	if (enable)
		write_cmos_sensor(0x0100, 0X01);
	else
		write_cmos_sensor(0x0100, 0x00);
