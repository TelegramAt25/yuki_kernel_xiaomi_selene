...
#include "s5k3l6_qtech_mipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3L6_QTECH_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[7] = {
 ...
 { 4208, 3120,	824,  840, 2560, 1440, 1280,  720,   0,	0, 1280,  720,	 0, 0, 1280,  720}, //hight speed video
...
static void write_shutter(kal_uint64 shutter)
{
	...
	if (shutter <= 65530) {
		if (bNeedSetNormalMode) {
			...
			while (1) {
				mdelay(1);
				timeout++;
				if ((read_cmos_sensor_8(0x0005) == 0xFF) || (timeout > 2000))
					break;
			}
			...
			write_cmos_sensor_8(0x3C1E, 0x01);      //stream on
	...
	LOG_DBG("shutter =%d, framelength =%d\n",
		...
		} else if (22200 < exposure_time) {
			temp1_0342 = 0x3f;
			temp2_0343 = 0x90;
			long_shutter_linelenght = 17216;
			LOG_INF(" > 22.2s exposure_time = %d\n", exposure_time);
			if (23000 < exposure_time) {
				shutter = 2300000;
				exposure_time = 23000;
				LOG_INF(" > 23.5s exposure_time = %d\n", exposure_time);
			}
		}
		...
		write_cmos_sensor_8(0x030E, 0x00);
		write_cmos_sensor_8(0x030F, 0x78);
		...
		write_cmos_sensor_8(0x38C3, 0x06);
		...
		write_cmos_sensor_8(0x0342, temp1_0342);
		write_cmos_sensor_8(0x0343, temp2_0343);
...
static struct s5k3l6_otp_data_str pOTP_Data = {0};

static kal_uint32 read_eeprom_info_data(
		struct s5k3l6_otp_data_str *pData, kal_uint16 addr)
{
...
static kal_uint32 s5k3l6_otp_apply(struct s5k3l6_otp_data_str *pData)
{
	/* Apply AWB */
...
static void s5k3l6_get_eeprom_data(void)
{
 ...
 *
 *************************************************************************/
//modify for camera rlkdevinfo by wei.miao@reallytek.com 20180529 start
#if defined(CONFIG_TRAN_SYSTEM_DEVINFO)
extern void app_get_back_sensor_name(char *back_name);
#endif
//modify for camera rlkdevinfo by wei.miao@reallytek.com 20180529 end
...
UINT32 S5K3L6_QTECH_MIPI_RAW_SensorInit(
