...
static enum IMGSENSOR_RETURN gpio_release(void *pinstance)
{
	...
	for (j = IMGSENSOR_SENSOR_IDX_MIN_NUM;
		...
		for (i = GPIO_CTRL_STATE_PDN_L;
		     i < GPIO_CTRL_STATE_MAX_NUM_CAM; i += 2) {
			lookup_names =
			...
			if (lookup_names != NULL &&
			    ...
			    pinctrl_select_state(pgpio->ppinctrl,
						 pgpio->
						 ppinctrl_state_cam[j][i])) {
				pr_debug("%s : pinctrl err, PinIdx %d name %s\n",
					__func__, i, lookup_names);
			}
	...
	for (i = 0; i < GPIO_CTRL_STATE_MAX_NUM_SWITCH; i++) {
		if (gpio_pinctrl_list_switch[i].ppinctrl_lookup_names) {
			pgpio->ppinctrl_state_switch[i] =
			    pinctrl_lookup_state(pgpio->ppinctrl,
						 gpio_pinctrl_list_switch[i].
						 ppinctrl_lookup_names);
		}
				      ...
				      enum IMGSENSOR_HW_PIN_STATE pin_state)
{
	...
	if (pin == IMGSENSOR_HW_PIN_MIPI_SWITCH_EN)
		ppinctrl_state =
		    pgpio->
		    ppinctrl_state_switch[GPIO_CTRL_STATE_MIPI_SWITCH_EN_H +
					  gpio_state];

	else if (pin == IMGSENSOR_HW_PIN_MIPI_SWITCH_SEL)
		ppinctrl_state =
		    pgpio->
		    ppinctrl_state_switch[GPIO_CTRL_STATE_MIPI_SWITCH_SEL_H +
					  gpio_state];
	...
	{
		ppinctrl_state =
		    pgpio->ppinctrl_state_cam[sensor_idx][((pin -
...
enum IMGSENSOR_RETURN imgsensor_hw_gpio_open(struct IMGSENSOR_HW_DEVICE
					     **pdevice)
{
	*pdevice = &device;
	return IMGSENSOR_RETURN_SUCCESS;
